...
if __name__ == "__main__":
    ...
    model_name = "transformer_encoder_masked_mlp_decoder"
    ...
    for i in range(n_epochs):
        ...
        for i, (features, gt_edges) in pbar:
            ...
            features_mask = generate_square_subsequent_mask(features.shape[2] * features.shape[3], features.shape[2] * features.shape[3]).to(device)

            # features: [batch_size, num_objects, num_timesteps, num_feature_per_obj]
            logits = encoder.forward(features, rel_rec=rel_rec, rel_send=rel_send, mask=features_mask)
